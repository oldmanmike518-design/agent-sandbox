...
- [x] Publish acceptable-use, privacy, retention, and internal-credit disclaimers; document IP/user-agent logging and define retention/deletion (completed 2026-07-16: `PRIVACY.md` + `ACCEPTABLE_USE.md`, linked from README, `/llms.txt`, and the agent manifest). Event logs (IP/user-agent) now have a configurable `EVENT_LOG_RETENTION_DAYS` (default 90) enforced by `scripts/purge_old_events.py`; a purge integration test runs in CI. **Maintainer decision required:** set a real data-controller contact in place of `<CONTACT_EMAIL>` (a personal email was intentionally not committed), and schedule the purge job as a Render cron/periodic task during deployment verification.
